...
echo "master.njalnapure.com" > files/slaves
i=1
while [ $i -lt $N ]
do
	echo "slave$i.njalnapure.com" >> files/slaves
	((i++))
done
...
sudo docker rmi njalnapure/hadoop-master:$tag

# rebuild hadoop-master image
pwd
sudo docker build -t njalnapure/hadoop-master:$tag .
